...
Windows CI의 TypeScript 테스트 20개와 Rust 테스트 5개가 통과했다. 개인 원본 XLSX가 필요한 테스트 2개는 원본이 없는 환경에서 명시적으로 건너뛰도록 바꿨다. 설치 파일에는 WebView2 오프라인 설치 패키지를 포함한다. Windows 노트북에서 실제 설치·GUI 조작·Ctrl+S·오프라인 첫 실행을 확인한 것은 아니며, 코드 서명도 아직 없다. [설치 안내](WINDOWS_INSTALL.md)에 사용자가 확인할 순서를 기록했다.

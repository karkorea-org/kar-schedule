...
1. GitHub에 로그인하고 [빌드 실행 목록](https://github.com/karkorea-org/kar-schedule/actions/workflows/desktop-build.yml)을 엽니다. 비공개 저장소이므로 접근 가능한 계정이 필요합니다.
2. 초록색으로 성공한 실행을 열고, 아래 **Artifacts → KAR-Schedule-x86_64-pc-windows-msvc**를 다운로드합니다.
3. ZIP을 풀고 안의 `KAR Schedule_2.0.2_x64-setup.exe`를 실행해 설치합니다. 빌드 버전에 따라 숫자는 바뀝니다.
4. 시작 메뉴에서 **KAR Schedule**을 실행합니다. 새 PC는 빈 일정으로 시작합니다.
5. 상단 **Excel 연결**로 노트북의 본인 업무 일지 `.xlsx`를 선택하고 일정 시트와 연도를 확인합니다.
6. 업무를 수정하고 **저장 / Ctrl+S**를 누릅니다. 저장 완료를 확인한 뒤 FreeFileSync로 NAS에 동기화합니다.
...
Actions → Desktop test installers → Run workflow에서 `platforms: windows`를 선택합니다. GitHub의 Windows 환경에서 의존성 설치, TypeScript 테스트, Rust 테스트, NSIS 설치 파일 생성을 순서대로 진행합니다. macOS 설치 파일도 필요할 때 `all`을 선택합니다.
